# -*- coding: utf-8 -*-
"""
Laplateforme.com STABLE scraper - NO PARALLELISM
Reliable, CSV always synced, logs always work!
"""
...
class LaplatefFormeStableScraper:
    ...
    def log(self, msg):
        ...
        sys.stdout.flush()  # DOUBLE FLUSH for Windows!
                ...
                for cat_i, cat_url in enumerate(categories, 1):
                    ...
                    for i, url in enumerate(cat_product_urls, 1):
                        ...
                        try:
                            ...
                            # Log with aligned columns: name (35 chars), 2 prices, PDF status
                            sku = product['sku']
                            ...
                            pdf_status = "✓" if product['pdf_downloaded'] == 'yes' else "✗"
                            
                            self.log(f"    [{total}] [{sku}] {status_icon} {name_short} | {price_main} / {price_ht} | PDF: {pdf_status}")
                            ...
                            self.log(f"    [{len(self.products)+1}] ✗ ERROR: {str(e)[:60]}")
